...
def vector_cross_entropy(z, x):
    """Vector Cross Entropy measures cross entropy of a multivariate variable.

  H(z,x)=- sum_i (p(x_i)*log(p(z_i)))
  as inputs are probs.:
  H(z,x)=- sum_i (x_i*log(z_i))
...
def conv1d(x, n_output,
           k_w=5, d_w=2, padding='SAME', name='conv1d', reuse=None):
    ...
    with tf.variable_scope(name or 'conv1d', reuse=reuse):
        W = tf.get_variable(
            name='W',
            shape=[k_w, x.get_shape()[-1], n_output], #filter_width, inputchannels, outputChannels
            initializer=tf.contrib.layers.xavier_initializer()) # xavier_initializer_conv2d())
...
def linear(x, n_output, name=None, activation=None, reuse=None):
    ...
    with tf.variable_scope(name or "fc", reuse=reuse):
        ...
        if activation:
            h = activation(h, name='activationOperationForOutput')
